...
<div class="container" style="margin-top: 90px;">
      ...
      <div class="section"></div>
          <div class="col s4 offset-s4 z-depth-1 teal accent-2 animated bounce">
            <div class="transparent" style="font-family: Helvetica;"><h3 style="text-align: center;">Login Admin</h3></div>
...
<?php 
    if (isset($_POST['login'])) {
        $user = $_POST['user'];
        $pass = $_POST['pass'];
        $query = mysqli_query($conn, "select * from daftar_admin where user='$user' and pass='$pass'");
        $result = mysqli_fetch_array($query);
            if (mysqli_num_rows($query) == 1) {
                session_start();
                $_SESSION['logged_admin'] = $result;
